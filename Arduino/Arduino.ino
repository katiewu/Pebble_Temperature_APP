...
#define HOT (30)       /* Hot temperature, drive red LED (27c) */
int cold = 23;
int hot = 30;
...
{ 
  ...
  while (1)
  {
    ...
    input = Serial.read();
    // c, f, p, r
    //change the mode to pause
    if(input == 'p'){
      flag_pause = true;
    }
    //change the mode to resume
    if(input == 'r'){
      flag_pause = false;
    }
    // change the mode to convert
    if(input == 'c'){
      flag_c = true;
    }
    //change from c to f
    ...
    //change season mode to summer
    if(input == 's'){
      cold = 23;
      hot = 30;
    }
    //change season mode to winter
    if(input == 'w'){
      cold = 10;
      hot = 0;
    }
...
//change from c to f
void C_TO_F (int& Decimal, byte& Temperature_H){
 float temp = Decimal/10000.0 + Temperature_H;
 temp = temp * 9.0 / 5.0 + 32.0 + 0.5;
 Temperature_H = (int) temp;
 Decimal = (temp - Temperature_H) * 10000.0; 
}
